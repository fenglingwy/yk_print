...
//					drawVerticalSeparator(utils);
    ...
    private void drawRowContent(CanvasUtils utils, PrintData3 printData) {
        ...
        utils.drawText(top_left_x, top_left_y + row_height[0] * 5 + row_height[1], top_right_x,
                top_left_y + row_height[1] * 2 + row_height[0] * 5, " 出库箱号：",
                24, CanvasUtils.PAlign.ALIGN_LEFT, 0);

        utils.drawBarCode(top_left_x + row36_column1_width + 60, top_left_y + row_height[0] * 5 + row_height[1],
                top_right_x, top_left_y + row_height[0] * 5 + row_height[1] * 2,
